...
export function BottleNutrition({ data, allergensSummary }: Props) {
  ...
  // Build sub-label summary
  const subParts: string[] = [];
  if (hasNutrition) subParts.push('Nutritional');
  if (data || allergensSummary) subParts.push('Allergens');
  if (hasStorage) subParts.push('Storage');
  if (hasDeclarations || hasTechInfo) subParts.push('Declarations');
    ...
    <section>
      ...
      >
        ...
        <div className="flex flex-col items-center">
          ...
          <span
            className="font-sans-consumer"
            style={{ fontSize: 10, fontWeight: 300, color: '#9a9a9a', marginTop: 2 }}
          >
            {subLabel}
          </span>
        </div>
              ...
              {hasDeclarations && (
                <>
                  ...
                  <div className="space-y-0">
                    {data.gmo_declaration && <DataRow label="GMO" value={data.gmo_declaration} />}
                    {data.ionising_radiation && <DataRow label="Ionising Radiation" value={data.ionising_radiation} />}
                    {data.compliance_references && <DataRow label="Compliance" value={data.compliance_references} />}
                  </div>
                </>
              )}
