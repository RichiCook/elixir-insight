-- ============================================================
-- Collaboration core-cocktail customization — 2026-06-11
--
-- Goal: let a collab brand fork a shared CORE cocktail into its own
-- independently-editable product (own images / assets / layout) WITHOUT
-- changing the public URL, and without touching the original Classy product.
--
-- Mechanism (client side, useCustomizeCoreCocktail): clone the product +
-- its content into a new collab-owned product with a UNIQUE internal slug
-- (e.g. "margarita-alevi"), then repoint the collaboration_cocktails row at
-- the clone. To keep the shareable URL clean and STABLE across the fork, the
-- join row remembers the original public slug here:
--
--   collaboration_cocktails.public_slug
--     NULL  -> use the linked product's own slug (shared core, signature)
--     set   -> the public URL segment, independent of the product's slug
--
-- The bottle-page RPC resolves the collab path on COALESCE(public_slug, slug)
-- so /b/alevi/margarita works the same before AND after customization.
-- ============================================================

ALTER TABLE public.collaboration_cocktails
  ADD COLUMN IF NOT EXISTS public_slug text;

COMMENT ON COLUMN public.collaboration_cocktails.public_slug IS
  'Public URL segment for /b/<collab>/<public_slug>. NULL = fall back to the linked product slug. Set when a core cocktail is forked so the shareable link stays stable.';

-- ── Recreate get_bottle_page_data: PATH 2 now matches on the public slug ─────
-- (Only the PATH 2 product-resolution WHERE clause changed vs 20260610000001;
--  full body reproduced because Postgres has no partial function patch.)
